...
const MAX_PYTHON_PROCESSES = 20;
const MAX_PYTHON_QUEUE = 200;
const pythonQueue: Array<() => void> = [];

function schedulePythonTask(task: () => Promise<void>): boolean {
  const run = () => {
  ...
  if (activePythonProcesses >= MAX_PYTHON_PROCESSES) {
    if (pythonQueue.length >= MAX_PYTHON_QUEUE) return false;
    pythonQueue.push(run);
    return true;
  }
  run();
  return true;
...
function logVerbose(target: WecomWebhookTarget, message: string): void {
  const should =
    ...
    (() => {
      try {
        return getWecomRuntime().logging?.shouldLogVerbose() ?? false;
      } catch {
        return false;
      }
    })();
...
async function startAgentForStream(params: {
  ...
  // [PATCH] Save media if present so Python script can access it
  if (media && media.buffer) {
      try {
          const saved = await core.channel.media.saveMediaBuffer(
              ...
              media.contentType,
              "inbound",
              resolveWecomMediaMaxBytes(config),
              media.filename
          );
          // Append FILE_PATH tag
          rawBody += ` [FILE_PATH: ${saved.path}]`;
          
          // Also check if we should use --analyze-only for pure images?
          // For now, let the python script decide based on presence of FILE_PATH
      } catch (err) {
          logVerbose(target, `failed to save inbound media: ${err}`);
      }
  }

  // [PATCH] 直接接入 HDB RAG (Direct HDB RAG Integration)
  // 用户要求：手机端机器人的所有请求都直接根据知识库回答，不经过复杂的 Agent 思考。
  // 实现方式：Monitor 直接调用 Python 脚本查询 HDB RAG (DeepSeek/Zhipu)，返回结果。
  
  if (rawBody.trim()) {
    const accepted = schedulePythonTask(async () => {
      try {
      const { spawn } = await import("node:child_process");
      
      // 简单的防注入处理 (实际应更严谨，但此处为内部脚本调用)
      const safeQuery = rawBody.replace(/"/g, '\\"');
      const scriptPath = "/root/.openclaw/workspace/skills/hdb-rag/ask_rag.py";
      
      // Detect voice request
      const useVoice = rawBody.includes("语音") || rawBody.includes("播报") || rawBody.includes("说");
      
      logVerbose(target, `hdb-rag-direct: 调用脚本查询 query="${safeQuery}" voice=${useVoice} rawBody_len=${rawBody.length}`);
      
      // Use spawn for streaming output
      const child = spawn("python3", [scriptPath, safeQuery, ...(useVoice ? ["--voice"] : [])]);
        ...
        child.stdout.on("data", (data) => {
            ...
            // Check for status update (keep existing logic for simple status check)
            if (data.includes("[STATUS: SEARCHING_KB]")) {
                 streamStore.updateStream(streamId, (s) => {
                    s.content = "惠电宝正在为您查询知识库...";
                 });
                 // Explicitly push update to WeCom
                 const responseUrl = getActiveReplyUrl(streamId);
                 if (responseUrl) {
                     const searchingReply = buildStreamReplyFromState(streamStore.getStream(streamId)!) as unknown as Record<string, unknown>;
                     useActiveReplyOnce(streamId, async ({ responseUrl, proxyUrl }) => {
                        await wecomFetch(responseUrl, {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
                            body: JSON.stringify(searchingReply),
                        }, { proxyUrl, timeoutMs: LIMITS.REQUEST_TIMEOUT_MS });
                     }).catch(() => {});
                 }
            }

            // Handle Streaming Chunks [STREAM] "chunk"
            ...
            for (const line of lines) {
                ...
                if (trimmed.startsWith("[STREAM] ")) {
                    try {
                        const jsonStr = trimmed.substring(9);
            ...
            if (hasNewContent) {
                streamStore.updateStream(streamId, (s) => {
                    s.content = accumulatedAnswer;
                });

                // Throttle updates to WeCom (every 800ms to balance smoothness and rate limits)
                const now = Date.now();
                if (now - lastPushTime > 800) {
                    lastPushTime = now;
                    const responseUrl = getActiveReplyUrl(streamId);
                    if (responseUrl) {
                        const streamingReply = buildStreamReplyFromState(streamStore.getStream(streamId)!) as unknown as Record<string, unknown>;
                        useActiveReplyOnce(streamId, async ({ responseUrl, proxyUrl }) => {
                            await wecomFetch(responseUrl, {
                                method: "POST",
                                headers: { "Content-Type": "application/json" },
                                body: JSON.stringify(streamingReply),
                            }, { proxyUrl, timeoutMs: LIMITS.REQUEST_TIMEOUT_MS });
                        }).catch(() => {});
                    }
                }
        ...
        await new Promise<void>((resolve, reject) => {
            child.on("close", (code) => {
                ...
                if (code === 0) resolve();
                else resolve(); // Treat error as empty answer to handle gracefully
            });
            child.on("error", (err) => resolve()); // Handle spawn error
            // Set timeout
            setTimeout(() => {
                child.kill();
                resolve();
        ...
        if (stderr) {
            logVerbose(target, `hdb-rag-direct: stderr output: ${stderr}`);
          }

          let answer = accumulatedAnswer.trim();
          if (!answer) {
               answer = stdout.replace("[STATUS: SEARCHING_KB]", "").trim();
               // Remove any [STREAM] lines from stdout fallback
               answer = answer.replace(/\[STREAM\] .*(\r?\n|$)/g, "").trim();
          }
        
        // Parse [AUDIO_FILE: path]
        let audioPath = "";
        const audioMatch = answer.match(/\[AUDIO_FILE: (.*?)\]/);
        if (audioMatch) {
            audioPath = audioMatch[1].trim();
            answer = answer.replace(audioMatch[0], "").trim();
        }
        
        // 如果脚本没有任何输出，或者脚本报错但没抛出异常
        if (!answer && !audioPath) {
             answer = "知识库服务未返回任何内容，请联系管理员检查日志。";
             if (stderr) answer += ` (Stderr: ${stderr})`;
        }

        // 无论成功还是失败，都直接回复用户，**绝不回落到 Agent**
        streamStore.updateStream(streamId, (s) => {
          s.finished = true;
          s.content = answer || (audioPath ? "正在发送语音..." : "无内容");
        });
        ...
        if (answer) {
            ...
            if (responseUrl) {
                ...
                logVerbose(target, `hdb-rag-direct: 已直接回复用户: ${answer.slice(0, 50)}...`);
            }
        }
        ...
        if (audioPath) {
             try {
                 ...
                 // If target is Agent
                 if ('agent' in target) {
                     const agentAccount = (target as any).agent;
                     const mediaId = await uploadMedia({
                         agent: agentAccount,
                         type: "voice",
                         buffer: audioBuffer,
                     ...
                     // Send via Agent Proactive API
                     await sendAgentMedia({
                         agent: agentAccount,
                         toUser: chatType === "direct" ? userid : undefined,
                         chatId: chatType === "group" ? chatId : undefined,
                         mediaId: mediaId,
                         mediaType: "voice"
                     });
                     logVerbose(target, `hdb-rag-direct: 已发送语音回复: ${audioPath}`);
                 }
                 ...
                 logVerbose(target, `hdb-rag-direct: audio send failed: ${e}`);
             }
        }
        
        // 结束处理
        streamStore.onStreamFinished(streamId);
        return; // <--- 强制结束，不让 Agent 接管
      
      } catch (err: any) {
        target.runtime.error?.(`hdb-rag-direct: 调用失败: ${String(err)}`);
        
        // 即使发生异常，也给用户一个错误提示，而不是回落到 Agent
        const errorMsg = `知识库服务调用异常: ${String(err)}`;
        streamStore.updateStream(streamId, (s) => {
            s.finished = true;
            s.content = errorMsg;
        });
        ...
        if (responseUrl) {
            const finalReply = buildStreamReplyFromState(streamStore.getStream(streamId)!) as unknown as Record<string, unknown>;
            await useActiveReplyOnce(streamId, async ({ responseUrl, proxyUrl }) => {
                await wecomFetch(responseUrl, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(finalReply) }, { proxyUrl, timeoutMs: LIMITS.REQUEST_TIMEOUT_MS });
            });
        }
        streamStore.onStreamFinished(streamId);
        return; // <--- 强制结束
      ...
      if (!accepted) {
        logVerbose(target, `hdb-rag-direct: Dropped request due to high load (${activePythonProcesses}/${MAX_PYTHON_PROCESSES})`);
        streamStore.updateStream(streamId, (s) => {
          s.finished = true;
          s.content = "系统繁忙，请稍后再试 (System Busy)";
        });
        streamStore.onStreamFinished(streamId);
        return;
      }
