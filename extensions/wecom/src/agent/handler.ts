    ...
    if (["image", "voice", "video", "file"].includes(msgType)) {
        ...
        if (mediaId) {
            try {
                ...
                // 针对图片消息，直接调用本地 Python 脚本进行多模态分析，并将结果注入到消息上下文中
                // 确保 LLM 能够“看到”图片内容，而不需要依赖不稳定的 Tool Use
                if (msgType === "image" && mediaPath) {
                    try {
                        log?.(`[wecom-agent] analyzing image: ${mediaPath}`);
                        // 构造命令，传入图片路径和用户文本（作为上下文）
                        // 注意：需要转义双引号以防止 shell 注入或截断
                        const safeContent = (content || "").replace(/"/g, '\\"');
                        const cmd = `python3 /root/.openclaw/workspace/skills/hdb-rag/ask_rag.py "Context: ${safeContent} [FILE_PATH: ${mediaPath}]" --analyze-only`;
                        
                        // 执行脚本，设置 45秒 超时（智谱 API 有时较慢）
                        const { stdout, stderr } = await execAsync(cmd, { timeout: 45000 });
                        const analysis = stdout.trim();
                        
                        if (analysis) {
                             // 将分析结果注入到 finalContent
                             finalContent += `\n\n[系统注入: 图片分析结果 (Visual Analysis)]\n${analysis}\n[系统提示: 请根据以上视觉分析结果回答用户问题]`;
                             log?.(`[wecom-agent] image analysis appended. len=${analysis.length}`);
                        } else {
                             log?.(`[wecom-agent] image analysis empty. stderr=${stderr}`);
                             // 如果分析失败，注入错误提示，避免 LLM 幻觉
                             finalContent += `\n\n[系统提示: 图片分析失败，请提示用户重新上传或描述图片内容]`;
                        }
                    } catch (err) {
                        error?.(`[wecom-agent] image analysis failed: ${err}`);
                        finalContent += `\n\n[系统提示: 图片分析服务暂时不可用 (${String(err)})]`;
                    }
                }
